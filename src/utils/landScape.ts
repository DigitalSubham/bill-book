...
.container { padding: 4px; width: 48%; }
...
<div class="subtotal">
  ...
  Total ${currency(invoice.taxableAmount)}
</div>
...
<div class="bank section-safe">
  <div class="bank-flex">
    ...
    <div class="amount-box">
      <div class="amount-row"><span class="label">Taxable:</span> ₹${safe(
        invoice.taxableAmount || '0.00',
      )}</div>
      <div class="amount-row"><span class="label">CGST:</span> ₹${safe(
        invoice.cgstTotal || '0.00',
      )}</div>
      <div class="amount-row"><span class="label">SGST:</span> ₹${safe(
        invoice.sgstTotal || '0.00',
      )}</div>
      <div class="amount-row total"><span class="label">Total:</span> ₹${safe(
        invoice.totalAmount || '0.00',
      )}</div>
      <div class="amount-row"><span class="label">Received:</span>
      ₹${safe(invoice.receivedAmount || '0.00')}</div>
    </div>
  </div>
</div>
